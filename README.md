...
这是一个**完整的单机逃生游戏 Demo**，完全从零自学 UE5 5 个月后的第一个完整项目。

**游戏核心玩法**：玩家被困在一个动态场景中，需要通过以下步骤逃脱：
- 🔍 **搜索并装配武器** - 在环境中寻找散落的武器部件，组合成可用的武器
- 🎒 **背包物品管理** - 实现物品拾取、存储、快速切换机制
- 🚪 **环境交互系统** - 与门、障碍物交互，制造逃脱机会
- ⚔️ **战斗与躲避** - 使用武器对抗敌人或选择躲避
- 🏃 **寻找逃生出口** - 在有限时间内找到并抵达安全地点

**设计理念**：
- 不是为了做一个"完整商业游戏"，而是在真实项目中**实践所有学到的知识**
- 所有核心逻辑用 **C++ 实现**（约 90%），部分 UI 和动画通过 **Blueprint** 补充
- 强调**组件化架构**和**代码复用性**，而非美术资源堆砌
...
| 功能模块 | 描述 | 技术亮点 |
|---------|------|--------|
| **角色控制** | 支持移动、跳跃、加速、蹲下等基础操作 | Enhanced Input System 事件驱动 |
| **视角系统** | 一/三人称平滑切换，武器状态自动调整视角 | 摄像机插值 + 状态机管理 |
| **背包系统** | 组件 + 数据资产驱动，支持拾取、存储、快速切换 | 配置化物品定义，易于扩展 |
| **武器系统** | 动态生成、装配、碰撞判定、动画蒙太奇驱动 | 蒙太奇回调同步伤害计算 |
| **交互系统** | Overlap 检测 + 委托通知，进门时限制操作并调整状态 | 非阻塞式事件通信 |
| **敌人 AI** | 巡逻、发现、追击、攻击四阶段状态机 | 距离感知 + 碰撞检测联动 |
| **血量系统** | 组件化管理，伤害计算与 UI 实时联动 | 委托模式解耦游戏逻辑 |
| **反馈系统** | 血条、伤害数字、动画反馈、屏幕抖动 | 多渠道综合反馈提升体验 |
